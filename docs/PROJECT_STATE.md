...
- **Pages** :
  ...
  - `/dashboard` — hub (13 liens : Organisation, Salon, Employés, Services, Horaires du salon, Jours de fermeture, Rendez-vous, Agenda, Clients, KPI & Tableau de bord, Caisse, **Stocks & Produits**)
...
- **Permissions** : `src/lib/permissions/` — `tenant.ts` + `organization.permissions.ts` + `salon.permissions.ts` + `employee.permissions.ts` + `service.permissions.ts` + `schedule.permissions.ts` + `appointment.permissions.ts` + `client.permissions.ts` + `payment.permissions.ts` + `inventory.permissions.ts`
- **Services métier** : `src/features/organizations/` + `src/features/salons/` + `src/features/employees/` + `src/features/services/` + `src/features/schedules/` + `src/features/appointments/` + `src/features/clients/` + `src/features/dashboard/` + `src/features/payments/` + `src/features/notifications/` + `src/features/inventory/`
- **Validation** : `zod@4.4.3` — Server Actions
- **Seed DEV** : `owner@test.local / Test1234!` (Organisation "Salon Test" + Salon "Salon Test").
- **Schéma Prisma** : 28 modèles + 16 enums + 8 migrations (4 appliquées + 1 en attente Docker + 3 new Sprint 14/15/16 en attente Docker).
...
- Branches feature/sprint9-agenda, docs/sprint9-closure, feature/sprint10-crm-clients, feature/sprint11-public-booking, docs/codex-guidelines, docs/codex-coauthor-rule, feature/sprint12-email-notifications, feature/sprint13-dashboard-kpi, feature/sprint14-payments-pos, feature/sprint15-reminders-receipts, feature/sprint16-inventory **supprimées** (locale + distante).
...
Sprint 17 : à définir avec ChatGPT.
...
_Dernière mise à jour : 2026-06-24 — PR #33 mergée, tag v1.7.0-inventory. Sprint 16 Gestion des Stocks & Produits TERMINÉ._
