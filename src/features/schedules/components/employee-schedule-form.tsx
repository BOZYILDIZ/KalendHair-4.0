        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left text-gray-600">
              ...
              <th className="pb-2 pr-4 font-medium">Travaille</th>
              <th className="pb-2 pr-4 font-medium">Début</th>
              <th className="pb-2 pr-4 font-medium">Fin</th>
              <th className="pb-2 font-medium text-gray-400">Salon</th>
            </tr>
          </thead>
          <tbody className="divide-y">
            {DAYS_OF_WEEK.map((day) => {
              const entry   = schedule.find((s) => s.dayOfWeek === day);
              const salon   = salonSchedule.find((s) => s.dayOfWeek === day);
              const startVal = minutesToTime(entry?.startMinute ?? salon?.startMinute ?? 540);
              const endVal   = minutesToTime(entry?.endMinute   ?? salon?.endMinute   ?? 1080);
              const isWorking = entry?.isWorking ?? false;
              const salonInfo = salon?.isOpen
                ? `${minutesToTime(salon.startMinute)}–${minutesToTime(salon.endMinute)}`
                : "Fermé";
              ...
              return (
                <tr key={day}>
                  ...
                  <td className="py-2 text-xs text-gray-400">{salonInfo}</td>
                </tr>
              );
            })}
          </tbody>
